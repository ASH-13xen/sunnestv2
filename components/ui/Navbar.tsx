...
export default function Navbar() {
  ...
  return (
    // `<nav>` itself must never carry a `filter`/`backdrop-filter` — either one
    // makes the element a new containing block for its `position: fixed`
    // descendants (the mobile drawer below), which silently shrinks the
    // drawer down to this bar's own ~60px height instead of covering the
    // viewport. That's why the drawer only rendered fully on the hero (nav
    // stays transparent there) and looked "cut off" everywhere else (nav
    // picks up `backdrop-blur-md` once scrolled past the hero). The blurred
    // background now lives on an inner bar wrapper instead.
    <nav className="fixed top-0 left-0 right-0 z-50">
      <div
        className={cn(
          "flex items-center justify-between px-5 md:px-8 py-3.5 transition-all duration-[400ms]",
          scrolled && !onHero
            ? theme === "night"
